/**
 * Alternativas financieras de una cotización, y su cálculo.
 *
 * Cuatro alternativas, ⛔ NO ACUMULABLES: el cliente elige una sola.
 *
 *   A · Plan estándar        permanencia 12 meses, sin descuento adicional
 *   B · Adelantado 12 meses  10 % sobre las 12 mensualidades especiales
 *   C · Adelantado 24 meses  20 % sobre las 24 mensualidades especiales
 *   D · Cheques diferidos o débito automático de tarjeta
 *                            10 % sobre la mensualidad especial, un mes
 *                            bonificado, permanencia 12 meses
 *
 ...
 *   B   S + (M × 12 × 0,90)
 *   C   S + (M × 24 × 0,80)
 *   D   S + (M × 11 × 0,90)      12 meses de servicio, 11 cuotas
 *
 * ⛔ El descuento se calcula sobre la MENSUALIDAD ESPECIAL. El setup se suma
 *    aparte y nunca recibe el descuento de la alternativa.
 * ⛔ Nunca se suman monedas distintas.
 * ⛔ Estas funciones son puras y compartidas: el cliente las usa para
 *    previsualizar, el servidor las vuelve a ejecutar antes de aprobar, y el
 *    resultado del servidor prevalece (API_CONTRACTS §0 A6).
 *
 * Ver MASTER_SPEC.md §10.8, COMMERCIAL_RULES.md §7, DATA_MODEL.md §7.6.
 */

import type { Dinero, Moneda } from './core';
...
// ---------------------------------------------------------------------------
// Lo que se muestra de cada alternativa
// ---------------------------------------------------------------------------
...
/** Ahorro del precio especial contra el de lista, para setup y mensualidad. */
export interface AhorroBase {
...
export function calcularAhorroBase(base: BaseCalculo): AhorroBase {
  const moneda = monedaDeLaBase(base);
  const ahorroSetup = base.setupLista.monto - base.setupEspecial.monto;
  const ahorroMensual = base.mensualLista.monto - base.mensualEspecial.monto;
  return {
    ahorroSetup: dinero(ahorroSetup, moneda),
    ahorroSetupPorcentaje:
      base.setupLista.monto === 0 ? 0 : (ahorroSetup / base.setupLista.monto) * 100,
    ahorroMensual: dinero(ahorroMensual, moneda),
    ahorroMensualPorcentaje:
      base.mensualLista.monto === 0 ? 0 : (ahorroMensual / base.mensualLista.monto) * 100,
  };
}
